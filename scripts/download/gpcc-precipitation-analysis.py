...
def main():
    print("Starting download workflow for gpcc-precipitation-analysis")
    setup_logging()
    dataset="gpcc-precipitation-analysis"

    logger.info(f"Starting download workflow for {dataset}")
    variables_file_path = f"../../requests_external/{dataset}.csv"
    download_files_external(dataset, variables_file_path)
