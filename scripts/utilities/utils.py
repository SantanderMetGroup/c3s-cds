...
def load_output_path_from_row(row, dataset=None, raw=False):
    """
...
def require_multiple_rows(df, mask, desc=None):
    """
    Select multiple rows from a DataFrame that match a given mask.
    ...
    -------
    pandas.DataFrame
        The rows from `df` that match the mask.
...
def require_rows(df, mask, desc=None , multiple=False):
    """
    Select rows from a DataFrame that match a given mask, requiring at least one match.
    ...
    mask : pandas.Series or array-like of bool
        Boolean mask used to filter rows in `df`.   
    desc : str, optional
        Description to include in the error message if no row is found.
    ...
    """
    try :
        single_match= require_single_row(df, mask, desc)
        return single_match,multiple
